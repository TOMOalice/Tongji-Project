###同理“171226_staticsorder”这个文件，这一类为121人，找出都不选效用以上的所有组合
...
ori2$beta1 <- ori1[1:10*2-1,2]
ori2$x2 <- ori1[1:10*2,1]
ori2$beta2 <- ori1[1:10*2,2]
ori2$delta <- abs(ori2$beta1-ori2$beta2)
ori2$big <- rec[1:10,2]
ori2$bbeta <- rec[1:10,3]
...
sn0[2,ncol(sn0)]<- (exp(ee)/(exp(ee)+exp(7.7832987)))
...
write.csv(sn1,"H:\\佳佳研究生\\09课程作业\\新建文件夹\\1219\\1.csv")
snn <- read.csv("H:\\佳佳研究生\\09课程作业\\新建文件夹\\1219\\1.csv",stringsAsFactors = FALSE)
snn <- snn[,-1]
...
write.csv(snn4,"H:\\佳佳研究生\\09课程作业\\新建文件夹\\1219\\paixu.csv")
